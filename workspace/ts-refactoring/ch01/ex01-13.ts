...
*/
  ...
  // ID 유효성 검사 예제
  let id: string = "honkjsdhfkjshdfiuwyehrifu";
  // ID 길이가 4보다 작거나 12보다 큰 경우 invalid
  let invalidId: boolean = id.length < 4 || id.length > 12;
  ...
  // OR 연산자 예제들
  // hello (앞의 값이 truthy)
  const result1: string = "hello";
  const result11: string = "world";

  // 90 (앞의 값이 truthy)
  const result2: number = 90;
  const result22: number = 30;

  // guset (앞의 값이 falsy)
  const result3: null = null;
  const result33: string = "guset";

  //guset (앞의 값이 falsy)
  const result4: undefined = undefined;
  const result44: string = "guset";

  // 30 (앞의 값이 falsy)
  const result5: number = 0;
  const result55: number = 30;

  // guset (앞의 값이 falsy)
  const result6: string = "";
  const result66: string = "guset";
